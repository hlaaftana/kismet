...
import hlaaftana.rewrite.kismet.exceptions.*
import hlaaftana.rewrite.kismet.parser.Parser
import hlaaftana.rewrite.kismet.vm.*
		...
		Map<String, Object> toConvert = [
				...
				'unique_via?': func { IKismetObject... args ->
					args[0].inner().invokeMethod('size', null) ==
							args[0].inner().invokeMethod('unique', [false, args[1].kismetClass().&call.curry(args[1])])
				},
				...
				let: /*new Template() {
					...
					Expression transform(Expression... args) {
						final definitions = args[0]
						def declarations = []
						if (definitions)
						final rest = args.length == 1 ? null : args.length args.tail()
						if (rest.length == 1 && rest[0] instanceof BlockExpression)
							rest
					}
				}*/macr { Context c, Expression... exprs ->
				...
				collect: func { IKismetObject... args -> args[0].inner().collect(args[1].kismetClass().&call.curry(args[1])) },
