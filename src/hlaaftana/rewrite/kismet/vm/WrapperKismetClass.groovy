...
import hlaaftana.rewrite.kismet.call.Function
import hlaaftana.rewrite.kismet.exceptions.CannotOperateException
import hlaaftana.rewrite.kismet.exceptions.ForbiddenAccessException
...
class WrapperKismetClass<T> implements IKismetClass<WrapperKismetObject> {
	...
	IKismetObject subscriptGet(WrapperKismetObject obj, IKismetObject key) {
		Kismet.model obj.inner().invokeMethod('getAt', key.inner())
	}

	IKismetObject subscriptSet(WrapperKismetObject obj, IKismetObject key, IKismetObject value) {
		Kismet.model obj.inner().invokeMethod('putAt', [key.inner(), value.inner()])
	}

	IKismetObject call(WrapperKismetObject obj, IKismetObject[] args) {
		Kismet.model obj.inner().invokeMethod('call', args*.inner())
	}
